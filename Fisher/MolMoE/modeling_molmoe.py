...
class MolmoeSparseMoeBlock(nn.Module):
    def __init__(self, config, layer_id):
        ...
        self.register_buffer('fisher_scores', torch.zeros(self.num_experts))  # To accumulate scores
        self.expert_counts = None  # To track counts per forward pass
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # Calculate expert counts
        selected_experts_flat = selected_experts.view(-1)
        self.expert_counts = torch.bincount(selected_experts_flat, minlength=self.num_experts)
